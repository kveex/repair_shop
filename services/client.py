...
def delete_client(supabase: Client, client_id: int) -> None:
...
def get_all_clients(supabase: Client) -> tuple[str, str, str, int]:
    ...
    for client in clients:
        ...
        print(f"ID: {c_id} | Имя: {name} | Телефон: {phone} | Адрес: {address}")

        result.append((name, phone, address, c_id))

    return result

def add_client(supabase: Client, name: str, phone: str, address: str = None) -> int:
    ...
    try:
        response = supabase.table("clients").insert({"name": name, "phone": phone, "address": address}).execute()
...
def get_client(supabase: Client, client_phone: str) -> tuple[str, str, str, int]:
    ...
    if client_exist:
        ...
        address: str = client_exist[0]["address"] if not client_exist[0]["address"] == None else "Не выдан"
